...
#define BD79124_INTERVAL_150 0
#define BD79124_INTERVAL_300 0
#define BD79124_INTERVAL_600 0
...
/*
 * The high limit, low limit and laste measurement result are each stored in
 ...
 */
static int bd79124_read_reg_to_int(struct bd79124_data *d, int reg, unsigned int *val)
{
	int ret;
 ...
 */
static int bd79124_write_int_to_reg(struct bd79124_data *d, int reg, unsigned int val)
{
	struct bd79124_raw raw;
	int ret, tmp;
...
static bool bd79124_chan_is_adc(struct bd79124_data *d, unsigned int offset)
{
	...
	/* If read fails, don't allow using as AIN (just to be on a safe side) */
	if (ret)
		return 0;
...
static int bd79124_start_measurement(struct bd79124_data *d, int chan)
{
	...
	/* Stop the measurement at the background */
	...
	/* re-start the measurement at the background */
	return regmap_update_bits(d->map, BD79124_REG_OPMODE_CFG,
				BD79124_MASK_CONV_MODE, BD79124_CONV_MODE_AUTO);
}
...
static int bd79124_stop_measurement(struct bd79124_data *d, int chan)
{
	...
	ret = regmap_write(d->map, BD79124_REG_AUTO_CHANNELS, (~BIT(chan)) & val);
	if (ret)
		return ret;

	/* Restart the measurement at the background */
	return regmap_update_bits(d->map, BD79124_REG_OPMODE_CFG,
				BD79124_MASK_CONV_MODE, BD79124_CONV_MODE_AUTO);
}
				      ...
				      enum iio_event_direction dir, bool state)
{
	...
	if (state) {
		bd79124_start_measurement(d, chan->channel);
		d->alarm_monitored[chan->channel] |= dir_bit;

		return regmap_set_bits(d->map, BD79124_REG_ALERT_CH_SEL,
				       BIT(chan->channel));
	} else {
		d->alarm_monitored[chan->channel] &= (~dir_bit);
		/*
		 ...
		 */
		if (!d->alarm_monitored[chan->channel]) {
			bd79124_stop_measurement(d, chan->channel);
...
static int bd79124_read_last_result(struct bd79124_data *d, int chan, int *result)
{
	struct bd79124_raw raw;
	int ret;
...
static int bd79124_hw_init(struct bd79124_data *d)
{
	...
	ret = regmap_update_bits(d->map, BD79124_REG_GEN_CFG,
				 BD79124_MASK_STATS_EN, BD79124_MASK_STATS_EN);
	if (ret)
		return ret;

	/* Set all channels to be auto-measured */
	ret = regmap_write(d->map, BD79124_REG_AUTO_CHANNELS, 0xff);
	if (ret)
		return ret;
	...
	ret = regmap_update_bits(d->map, BD79124_REG_OPMODE_CFG,
			BD79124_MASK_AUTO_INTERVAL, BD79124_INTERVAL_075);
	if (ret)
		return ret;
